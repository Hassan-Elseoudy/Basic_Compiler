		...
		for (i = 0; i < Data.size(); i++) {
			...
			if (Data.get(i).format.equals("4")) {
				...
				if ((c[1] - '0' >= 0 && c[1] - '0' <= 9)) { // #4096十轉十六->1000 放入hex
					String hex = Integer
							.toHexString(Integer.parseInt(Data.get(i).third.substring(1, Data.get(i).third.length())));
			...
			if (Data.get(i).second.equals("BYTE")) {
				String str = "";
				char c[] = Data.get(i).third
						.substring(Data.get(i).third.indexOf('\'') + 1, Data.get(i).third.length() - 1).toCharArray();
